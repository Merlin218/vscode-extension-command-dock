...
export class UIHandler {
   ...
   * 显示添加按钮的输入界面
   */
  public static async showAddButtonDialog(): Promise<void> {
    try {
      // 获取按钮名称
      ...
      // 保存配置
      await ConfigManager.addButton(button);
   ...
   * 显示删除按钮的选择界面
   */
  public static async showRemoveButtonDialog(): Promise<void> {
    try {
      ...
      const confirmation = await vscode.window.showWarningMessage(
        `确定要删除按钮 "${selectedItem.button.name}" 吗？`,
        { modal: true },
        '删除'
      );

      if (confirmation === '删除') {
        await ConfigManager.removeButton(selectedItem.button.id);
        ButtonManager.removeButton(selectedItem.button.id);
   ...
   * 显示编辑按钮的界面
   */
  public static async showEditButtonDialog(): Promise<void> {
    try {
      const buttons = ConfigManager.getButtons();
